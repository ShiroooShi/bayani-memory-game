...
class _HomePageState extends State<HomePage> {
  ...
  int? EbestTime;
  int? MbestTime;
  int? HbestTime; // Add HbestTime for HardPage
  ...
  void initState() {
    ...
    EbestTime = null;
    MbestTime = null;
    HbestTime = null; // Initialize HbestTime
  }
      ...
      switch (difficulty) {
        ...
        case 'hard':
          if (bestTimeHard == null || time < bestTimeHard!) {
            bestTimeHard = time;
            HbestTime = time; // Update HbestTime for HardPage
          } else if (HbestTime == bestTimeHard) {
            HbestTime = time; // Update HbestTime if user beats the current best time
          }
          break;
      }
    });
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                title: Text('Best Easy Time: ${EbestTime ?? "No Record"} sec'), // Display EbestTime
                onTap: () {
                  Navigator.pop(context); // Close the drawer
                },
              ),
              ListTile(
                title: Text('Medium: ${MbestTime ?? "No Record"} sec'), // Display MbestTime
                onTap: () {
                  Navigator.pop(context); // Close the drawer
                },
              ),
  ...
  void showNextTutorialDialog(BuildContext context) {
    showDialog(
      context: context,
      barrierDismissible: false, // Dialog won't close when clicking outside
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text(
            'How to Play:',
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Gap(20),
                    ElevatedButton(
                      onPressed: () => openEasyPage(context), // Navigate to EasyPage
                      child: const Text('Easy'),
                    ),
                    const Gap(12),
                    ElevatedButton(
                      onPressed: () => openMediumPage(context), // Navigate to MediumPage
                      child: Text('Medium'),
